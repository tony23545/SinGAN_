    ...
    if len(Gs) > 0:
        if mode == 'rand':
            ...
            for G,Z_opt,real_curr,real_next,noise_amp in zip(Gs,Zs,reals,reals[1:],NoiseAmp):
                if count == 0:
                    z = functions.generate_noise([1, Z_opt.shape[2] - 2 * pad_noise, Z_opt.shape[3] - 2 * pad_noise], num_samp=opt.num_sample, device=opt.device)
                    z = z.expand(opt.num_sample, 3, z.shape[2], z.shape[3])
                else:
                    z = functions.generate_noise([opt.nc_z,Z_opt.shape[2] - 2 * pad_noise, Z_opt.shape[3] - 2 * pad_noise], num_samp=opt.num_sample, device=opt.device)
                z = m_noise(z)
                ...
                G_zs = imresize(G_z[0],1/opt.scale_factor,opt)
                for i in range(1, G_z.shape[0]):
                    img = imresize(G_z[i],1/opt.scale_factor,opt)
                    G_zs = torch.cat((G_zs, img), 0)
                G_z = G_zs
